...
const statusOptions = ["ALL", "PENDING", "PROCESSING", "PAID", "FAILED", "EXPIRED", "CANCELLED"];
...
const data = await apiFetch<DepositHistoryResponse>(`/api/deposit/history?${params.toString()}`);
...
return deposits.filter((entry) => [entry.orderNumber, entry.referenceNo, entry.reference, entry.paymentProvider, entry.paymentMethod].filter(Boolean).join(" ").toLowerCase().includes(needle));
}, [deposits, search]);
...
description="Review your database-backed deposits with search, filtering, and pagination."
/>
<GlassCard className="space-y-4">
<div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
<div className="flex-1">
<label className="text-xs uppercase tracking-[0.2em] text-white/45">Search</label>
<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Order or reference" className="mt-2 w-full rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-white outline-none" />
</div>
...
<button type="button" onClick={() => void loadDeposits()} className="rounded-2xl border border-gold/30 bg-gold/10 px-4 py-3 text-sm font-semibold text-gold">Refresh</button>
</div>
...
<tbody>
{loading ? (
<tr><td colSpan={7} className="px-3 py-6 text-center">Loading deposits…</td></tr>
...
<div className="font-medium text-white">{entry.orderNumber}</div>
<div className="text-xs text-white/45">{entry.referenceNo || entry.reference}</div>
</td>
<td className="px-3 py-3">{entry.paymentProvider || entry.paymentMethod || "—"}</td>
...
<p className="text-sm text-white/60">Showing {filteredDeposits.length} of {pagination.total} deposits</p>
...
<div className="text-sm text-white/60"><Link href="/withdraw-history" className="text-gold hover:underline">View withdrawal history</Link></div>
</div>
);
}
