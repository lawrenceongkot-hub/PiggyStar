...
import { ADMIN_SESSION_COOKIE_NAME, PLAYER_SESSION_COOKIE_NAME } from "./src/lib/server/auth-keys";

const allowedOrigins = [
process.env.ADMIN_APP_URL || "http://localhost:4173",
...
const token = getAuthorizationToken(request) || getCookieValue(request, ADMIN_SESSION_COOKIE_NAME);
if (!token) {
...
const token = getAuthorizationToken(request) || getCookieValue(request, PLAYER_SESSION_COOKIE_NAME);
if (!token) {
return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
}
